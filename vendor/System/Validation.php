...
class Validation
{
    ...
    public function requiredFile($inputName, $customErrorMessage = null)
    {
        ...
        if (! $file->exists()) {
            $message = $customErrorMessage ?: sprintf('%s این مورد ضروری است', ucfirst($inputName));
            $this->addError($inputName, $message);
        }
    ...
    public function image($inputName, $customErrorMessage = null)
    {
        ...
        if (! $file->isImage()) {
            $message = $customErrorMessage ?: sprintf('%s این تصویر معتبر نیست', ucfirst($inputName));
            $this->addError($inputName, $message);
        }
    ...
    public function email($inputName, $customErrorMessage = null)
    {
        ...
        if (! filter_var($inputValue, FILTER_VALIDATE_EMAIL)) {
            $message = $customErrorMessage ?: sprintf('%s این ایمیل معتبر نیست', ucfirst($inputName));
            $this->addError($inputName, $message);
        }
    ...
    public function float($inputName, $customErrorMessage = null)
    {
        ...
        if (! is_float($inputValue)) {
            $message = $customErrorMessage ?: sprintf('%s تنها نوع اعداد و اعشار قابل پذیرش است', ucfirst($inputName));
            $this->addError($inputName, $message);
        }
    ...
    public function minLen($inputName, $length, $customErrorMessage = null)
    {
        ...
        if (strlen($inputValue) < $length) {
            $message = $customErrorMessage ?: sprintf('%s باید کمتر از  %d', ucfirst($inputName), $length);
            $this->addError($inputName, $message);
        }
    ...
    public function maxLen($inputName, $customErrorMessage = null)
    {
        ...
        if (strlen($inputValue) > $length) {
            $message = $customErrorMessage ?: sprintf('%s باید بیشتر از %d', ucfirst($inputName), $length);
            $this->addError($inputName, $message);
        }
    ...
    public function match($firstInput, $secondInput, $customErrorMessage = null)
    {
        ...
        if ($firstInputValue != $secondInputValue) {
            $message = $customErrorMessage ?: sprintf('%s باید برابر %s', ucfirst($secondInput), ucfirst($firstInput));
    ...
    public function unique($inputName, array $databaseData, $customErrorMessage = null)
    {
        ...
        if ($result) {
            $message = $customErrorMessage ?: sprintf('%s وجود دارد', ucfirst($inputName));
            $this->addError($inputName, $message);
        }
    }
