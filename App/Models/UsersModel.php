<?php

namespace App\Models;

use System\Model;

class UsersModel extends Model
{
     /**
     * Table name
     *
     * @var string
     */
    protected $table = 'users';

    /**
    * Get All Users
    *
    * @return array
    */
    public function all()
    {
        return $this->select('u.*', 'ug.name AS `group`')->from('users u')
                    ->join('LEFT JOIN users_groups ug ON u.users_group_id=ug.id')
                    ->orderBy('score','DESC')
                    ->fetchAll();
    }

    public function getCount()
    {
      return $this->select('COUNT(id) as total')
                              ->from('users')
                              ->fetch();

    }

     /**
     * Create New User
     *
     * @return void
     */
    public function create()
    {
        $image = $this->uploadImage();

        if ($image) {
            $this->data('image', $image);
        }

        $this->data('first_name', $this->request->post('first_name'))
             ->data('last_name', $this->request->post('last_name'))
             ->data('users_group_id', $this->request->post('users_group_id'))
             ->data('email', $this->request->post('email'))
             ->data('password', password_hash($this->request->post('password'), PASSWORD_DEFAULT))
             ->data('gender', $this->request->post('gender'))
             ->data('birthday', strtotime($this->request->post('birthday')))
             ->data('ip', $this->request->server('REMOTE_ADDR'))
             ->data('created', $now = time())
             ->data('status', 'enabled')
             ->data('code', sha1($now . mt_rand()))
             ->insert('users');
    }

     /**
     * Upload User Image
     *
     * @return string
     */
     private function uploadImage()
     {
         $image = $this->request->file('image');

         if (! $image->exists()) {
             return '';
         }

         return $image->moveTo($this->app->file->toPublic('images'));
     }

     /**
     * Update Users Record By Id
     *
     * @param int $id
     * @param int $usersGroupId
     * @return void
     */
    public function update($id, $usersGroupId = null)
    {
        $image = $this->uploadImage();

        if ($image) {
            $this->data('image', $image);
        }

        $password = $this->request->post('password');

        if ($password) {
            $this->data('password', password_hash($password, PASSWORD_DEFAULT));
        }

        if (is_null($usersGroupId)) {
            $usersGroupId = $this->request->post('users_group_id');
        }

        $this->data('first_name', $this->request->post('first_name'))
             ->data('last_name', $this->request->post('last_name'))
             ->data('users_group_id', $usersGroupId)
             ->data('email', $this->request->post('email'))
             ->data('status', $this->request->post('status'))
             ->data('gender', $this->request->post('gender'))
             ->data('birthday', strtotime($this->request->post('birthday')))
             ->where('id=?' , $id)
             ->update('users');
    }




      public function getEmail()
      {
               return $this->select('*')
                           ->from('users')
                           ->where("email=?", $this->request->post('email'))
                           ->fetch();
      }
      public function checkForgotPassword($recoverCode)
      {
        $is_time=time();
               return $this->select('*')
                           ->from('users')
                           ->where("recover_code=? AND recover_code_time>=?",$recoverCode,$is_time)
                           ->fetch();
      }

      public function updateRecoverCode($recoverCode,$useTime,$id)
      {

            $this->data('recover_code',$recoverCode)
                 ->data('recover_code_time',$useTime)
                 ->where('id=?' , $id)
                 ->update('users');

      }
      public function active($active_code)
      {

            $this->data('is_active',1)
                 ->where('active_code=?' , $active_code)
                 ->update('users');

      }

      public function forgotPasswordChangePassword($id)
      {

          $password = $this->request->post('password');

          if ($password) {
              $this->data('password', password_hash($password, PASSWORD_DEFAULT));
          }
          $this->data('recover_code_time',0)
               ->where('id=?' , $id)
               ->update('users');
      }

      public function onlineTime($userId)
      {

            $onlineTime=time()+200;
            $this->data('online_time',$onlineTime)
                 ->where('id=?' , $userId)
                 ->update('users');

      }

      public function readTime($userId,$readTime)
      {

            $onlineTime=$readTime+30;
            $this->data('read_time',$onlineTime)
                 ->where('id=?' , $userId)
                 ->update('users');

      }

      public function allOnline()
      {
        $timeServer=time();
        return $this->select('COUNT(id) as total')
                                ->from('users')
                                ->where('online_time >= ?',$timeServer)
                                ->fetch();
      }
      public function activeCodeUpdate($active_code,$user_id)
      {
        $create_time=time()+500;
            $this->data('active_code',$active_code)
                  ->data('active_code_time',$create_time)
                 ->where('id=?' ,$user_id)
                 ->update('users');

      }

}
